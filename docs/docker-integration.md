...
- 패키지는 필요한 CUDA 또는 WebGPU 가속기 계약을 선언할 수 있습니다.
...
- GPU가 필요하면 host/runtime이 실제 가속기를 제공하는지 먼저 확인한다.
...
CUDA는 브라우저가 직접 제공하는 기능이 아닙니다. Gaesup-State에서 말하는 CUDA 지원은 host/runtime이 CUDA 실행 계층을 제공하고, 컨테이너가 그 요구사항을 manifest로 선언하는 방식입니다.
...
이 경우 `onnxruntime-gpu` 같은 라이브러리는 컨테이너에 패키징됩니다. 반면 CUDA runtime 자체는 host/runtime이 제공해야 하며, `CompatibilityGuard`는 실행 전에 `cuda` 버전과 capability를 확인합니다.
